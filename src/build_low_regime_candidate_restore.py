...
SOURCE_COL = "low_regime_multistage_target15_repair_pred"
PRODUCTION_BASE_COL = "daybias31_hb22_midday_d8_b050_abs250_pred"
OUTPUT_COL = "low_regime_candidate_restore_target15_pred"

RESTORE_STEPS = [
    {
        "name": "low100_midchain_blend",
        "candidate_col": "morning7_10_hourweekend_after_night_sourcebin_pred",
        "hours": [10, 11, 12, 13, 14, 15, 16],
        "source_max": 100.0,
        "alpha": 0.75,
    },
    {
        "name": "h12_rollmin500_day14_restore",
    ...
    {
        "name": "h14_day13_restore",
        "candidate_col": "day13_16_ratio_wmape15_after_morning7_pred",
        "hours": [14],
        "alpha": 1.0,
    },
]
...
def _step_mask(frame, source, step):
    ...
    selected = hour.isin(step["hours"]).to_numpy()
    if "source_max" in step:
        selected &= source <= float(step["source_max"])
    if "rollmin_max" in step:
        rollmin = pd.to_numeric(frame["f_rolling_min_24"], errors="coerce").to_numpy(dtype="float64")
        selected &= rollmin <= float(step["rollmin_max"])
    return selected


def build_candidate_restore(predictions, source_col=SOURCE_COL, output_col=OUTPUT_COL):
    required = {"datetime", "actual", "price_cap", source_col, PRODUCTION_BASE_COL, "f_rolling_min_24"}
...
def main():
    parser = argparse.ArgumentParser(description="Restore selected prior candidates after multistage low-regime repair.")
    parser.add_argument(
        "--input-csv",
        default=os.path.join(ROOT_DIR, "output", "neural_experiment_low_regime_multistage_target15_repair_v1_predictions.csv"),
    )
    parser.add_argument("--output-dir", default=os.path.join(ROOT_DIR, "output"))
    parser.add_argument("--experiment-id", default="low_regime_candidate_restore_target15_v1")
    ...
    restored, summaries = build_candidate_restore(predictions, source_col=args.source_col, output_col=args.output_col)
    ...
    payload["restore_steps"] = summaries
