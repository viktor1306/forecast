...
def apply_multi_candidate_blend_adjustment(predictions, args):
    ...
    for candidate_col in candidate_cols:
        ...
        signal = shifted_candidate_advantage(
            frame=frame,
            source_col=args.source_col,
            candidate_col=candidate_col,
        ...
        candidate = frame[candidate_col].to_numpy(dtype="float64")
        finite_mask = np.isfinite(source) & np.isfinite(candidate) & np.isfinite(signal)
        valid = (
            finite_mask
            & hour_mask
            & (signal >= args.advantage_threshold)
            & distance_mask(source, candidate, cap, args.distance_op, args.distance_threshold)
        )
        signal_columns.append(np.where(valid, signal, -np.inf))
        candidate_arrays.append(candidate)
        valid_columns.append(valid)

    signal_matrix = np.column_stack(signal_columns)
    candidate_matrix = np.column_stack(candidate_arrays)
    best_idx = np.argmax(signal_matrix, axis=1)
    best_signal = signal_matrix[np.arange(len(frame)), best_idx]
    selected = np.isfinite(best_signal)
    best_candidate = candidate_matrix[np.arange(len(frame)), best_idx]

    adjusted = source.copy()
    adjusted[selected] = (1.0 - args.alpha) * source[selected] + args.alpha * best_candidate[selected]
    ...
    chosen_names[selected] = candidate_names[best_idx[selected]]
    chosen_index = np.full(len(frame), -1, dtype="int64")
    chosen_index[selected] = best_idx[selected]
    ...
    frame[f"{args.output_col}_candidate_index"] = chosen_index
    frame[f"{args.output_col}_candidate_advantage_signal"] = np.where(selected, best_signal, np.nan)
    for idx, candidate_col in enumerate(candidate_cols):
        frame[f"{args.output_col}_signal_{idx}"] = np.where(valid_columns[idx], signal_columns[idx], np.nan)
...
def append_log(
    ...
    with open(log_path, "a", encoding="utf-8") as f:
        ...
        f.write(
            "- Shifted multi-candidate blend adjuster: "
        ...
        f.write(
            "- Formula: each row chooses the candidate with the strongest shifted rolling historical advantage; "
            "selected rows use `(1-alpha) * source + alpha * candidate` and are clipped to the market range.\n"
        )
...
def main():
    parser = argparse.ArgumentParser(description="Leakage-safe shifted multi-candidate blend adjustment.")
    ...
    parser.add_argument("--output-col", default="multi_candidate_blend_adjusted_pred")
    ...
    adjusted, candidate_cols = apply_multi_candidate_blend_adjustment(predictions, args)
    variant_cols = [
    ...
    payload["multi_candidate_blend_adjuster_params"] = params
    payload["multi_candidate_blend_adjuster_applied_rows"] = applied_rows
    payload["multi_candidate_blend_adjuster_candidate_counts"] = candidate_counts
